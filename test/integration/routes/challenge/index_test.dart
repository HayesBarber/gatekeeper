...
void main() {
  ...
  group('GET /challenge', () {
    ...
    tearDown(() async {
      await redis.delete(ns: Namespace.challenges, key: TestEnv.clientId);
      await redis.delete(ns: Namespace.apiKeys, key: TestEnv.clientId);
    });
    ...
    test('filters out expired challenges', () async {
      ...
      await redis.set(
        ...
        value: ChallengeCollection(
          challenges: {expiredChallenge.challengeId: expiredChallenge},
        ).encode(),
      );
